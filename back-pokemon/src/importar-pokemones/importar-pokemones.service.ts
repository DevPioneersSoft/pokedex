...
import { ListaPokemon } from './dto/lista-pokemon-interfaces';
    ...
    try {
      ...
      const detalles = await Promise.allSettled(
        pokemones.results.map(async ({ url }) => {
          ...
          if (!resp.ok) {
            throw new Error(`Ha ocurrido un error ${url}`);
          }
          ...
          const speciesUrlFixed = speciesUrl.endsWith('/')
            ? speciesUrl.slice(0, -1)
            : speciesUrl;
          const speciesApi = await fetch(`${speciesUrlFixed}`);

          if (!speciesApi.ok) {
            throw new Error(
              `Ha ocurrido un error al consultar la especie del pokemon: ${pokemon.id}`,
            );
          }
      ...
      const tipos = new Set(
        detalles
          ...
          .flatMap((i) => i.value.pokemon.types)
          .map((i) => ({
            nombre: i.type.name,
          })),
      );
      ...
      Promise.all(
        detalles
          .filter((i) => i.status === 'fulfilled')
          .map(({ value }) =>
            this.registrarPokemon(value.pokemon, value.especie),
          ),
      );
    } catch (error) {}
  }

  async registrarPokemon(

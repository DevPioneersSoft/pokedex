...
export function useBuscarPokemones(hookParams?: UseBuscarPokemonesParams) {
	...
	/* 
    Alamacena valores para no volver a calcularlos cada vez que se ejecute
  */

	const pokemonsOrdenados = useMemo(() => {
		if (!query.data?.data) return [];
		const favoritosSet = new Set(hookParams?.favoritos ?? []);
		const favoritos = query.data.data.filter((p) => favoritosSet.has(p.id));
		const noFavoritos = query.data.data.filter((p) => !favoritosSet.has(p.id));

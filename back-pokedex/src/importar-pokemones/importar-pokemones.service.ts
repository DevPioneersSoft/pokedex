...
import { PokemonEspecies } from './dto/pokemon-species-interface';
...
export class ImportarPokemonesService implements OnModuleInit {
  ...
  try {
    ...
    const detalles = await Promise.allSettled(
      pokemones.results.map(async ({ url }) => {
        ...
        if (!resp.ok) {
          throw new Error('Ha ocurrido un error al consultar el pokemon');
        }
        ...
        const respSpecies = await fetch(`${speciesUrlFixed}`);
        if (!respSpecies.ok) {
          throw new Error(`Ha ocurrido un error al consultar la especie del pokemon: ${pokemon.id}`);
        }
        const especie = (await respSpecies.json()) as PokemonEspecies;
  ...
  async registrarPokemon(pokemon: PokemonDetalles, especie: PokemonEspecies): Promise<any> {

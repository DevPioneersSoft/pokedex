...
import { ListaPokemon } from './dto/lista-pokemon.interface.dto';
import { PokemonDetalles } from './dto/pokemon-detalles.interface.dto';
import { PokemonSpecies } from './dto/pokemon-species.interface.dto';
...
export class ImportarPokemonesService {
  ...
  async onModuleInit(){
    ...
    if (count === 0) {
      this.logger.log("Importando pokemones . . .");
      await this.importar();
      this.logger.log("Completado");
    } else {
      this.logger.log(`Error inesperado en ${count} .`);
    }
  }
  ...
  async importar() {
    try {
      ...
      if(!resp.ok){
        throw new Error('nimodo no funcó')
      }
      ...
      const detalles = await Promise.allSettled(
        pokemones.results.map(async ({ url }) => {
          const resp2 = await fetch(`${url.endsWith("/")? url.slice(0, -1) : url}`)
            if(!resp.ok){
              throw new Error(`nimodo no funcó la consulta del pokemon: ${url}`)
            }
      ...
      const tipos = new Set(detalles.filter(i => i.status === "fulfilled")
      .flatMap(i => i.value.pokemon.types).map(i => ({
        nombre: i.type.name
      })));
